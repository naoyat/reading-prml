...
;; グラフを表示する
(open-image "curve_9_*.png")


(let1 M 9
  (for-each (lambda (N)
              (set! D (generate-data gen N))
              (format #t "[M=~d, N=~d] " M N)
              (let1 w (array->vector (curve-fitting D M))
                (print "w:"  w)
                (make-graph-png (format #f "curve_~dN~d.png" M N) '(0 1 1) '(-2 2 1) (make-y w)
                                (lambda (im f x0 xF y0 yF obj)
                                  (let ([black [obj'black]]
                                        [green ([obj'color] 128 224 128)])
                                    (plot-data-points im f x0 xF y0 yF obj)
                                    ([obj'plot-curve] (lambda (x) (sin (* 2 pi x))) green)
                                    (gd-image-string+ im f (+ x0 15) (+ yF 7) (format #f "M = ~a" M) black)
                                    (gd-image-string+ im f (+ x0 15) (+ yF 21) (format #f "N = ~a" N) black))))))
            '(15 100)))

;; グラフを表示する
(open-image "curve_9N*.png")
